...
def build_subsidy_source_template() -> bytes:
    return _build_table_template(
        ...
        [("张三", "MT0001", "默认考勤组", "总部", "产品技术部", "后端组", "工程师",
          0, 0, 0, 0, 1, 50, 1, 50)],
        [
            "请上传从钉钉考勤后台导出的「月度汇总表（补贴及扣款）」Excel 文件。",
            "导出路径：钉钉考勤打卡 → 考勤统计 → 报表管理 → 月度汇总表（补贴及扣款）。",
...
def build_roster_template() -> bytes:
    return _build_table_template(
        ...
        [("张三",), ("李四",)],
        [
            "花名册只用于限定最终汇总需要处理的员工名单，姓名为唯一必填列。",
            "旧版包含工号、部门、岗位等扩展列的花名册仍可上传，但最终表身份与组织字段以钉钉月度考勤记录为准。",
            "姓名会去除首尾空格；空姓名会被跳过；重复姓名会因无法唯一匹配而中止。",
        ],
    )

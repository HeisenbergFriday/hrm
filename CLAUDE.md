...
- **组织模块四项验收修复完成**：员工类型映射由后端 `resolveHRMEmploymentType` 动态解析 HRM API 响应（含 org 级 `HRMFieldOptions` 覆盖），未知代码显示"未知类型（代码：X）"；`collation.go` 提供 MySQL `COLLATE utf8mb4_unicode_ci` 支持中文姓名模糊搜索；人数统计统一使用 `uniqueEmployeeSnapshots` 去重，三个页面数据源一致；试用期/转正预警卡片与列表共用 `isProbationEmployee`/`isRegularizationWarning` 谓词。所有 org handler（GetOrgOverview/GetOrgEmployees/GetOrgDepartmentTree/GetOrgEmployeeDetail/GetOrgDepartmentHistory）已绑定 `orgID` 防止跨租户查询泄漏。新增测试：`EmployeeList.test.tsx`（URL 状态 7 场景）、`org_service_regularization_test.go`（日期优先级 10 场景 + 一致性断言）、`org_service_mysql_integration_test.go`（中文搜索 7 场景）。

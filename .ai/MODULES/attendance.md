...
- 普通数字小时制请假跨相邻两个月时，以钉钉“系统时长”为总账，先按日生成基础分摊，再由结束日向前吸收小于 8 小时的边界/舍入差；各月分摊合计必须等于系统时长。差异达到 8 小时或无法守恒时保留原作息计算并在备注标记“需人工确认”。同月请假、产假、陪产假/丧假、文字“N天”和人工覆盖记录保持原口径；最终汇总的离职计薪日明细必须复用同一日分摊，禁止二次按作息表把已分摊时长丢失。

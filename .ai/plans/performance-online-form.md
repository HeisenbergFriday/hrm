...
- 按员工架构所属组织分组，优先按部门计算；部门人数或名额规则不足时再按中心汇总
- 员工占用其所属部门/中心的绩效名额，不占用项目部门或临时业务线名额
- 支持管理员提前录入部门/中心的比例、人数名额和等级组合上限，例如 3 人部门 `S/A` 合计最多 1 人、其他等级不限
- 逐部门/中心计算 S/A/B/C/D 或等级组合的当前数量和配额
- 返回结构：
  ...
  "scopes": [
    {
      ...
      "scope_id": "dept_001",
      "scope_name": "产品一部",
      "fallback_scope_type": "center",
      "fallback_scope_id": "center_001",
      "total": 8,
      "levels": {
...
- 分数自动换算出的等级只作为建议等级；当建议等级与强制分布冲突时，以强制分布为准，最终等级必须在可用名额内提交
- 页面提示文案需说明制度口径：部门考评总人数关联强制分布比例得出各等级人数，即使分数集中在同一区间，也需按评级比例强制排名分布
...
- 分布范围优先按员工架构所属部门计算，再汇总到中心口径。
- 小人数部门不按固定比例生硬取整，支持管理员按部门/中心提前统一录入名额或组合上限，例如 3 人部门 `S/A` 最多 1 人，其他等级不限。
- 若分数对应等级与比例分布冲突，以比例分布为准，分数等级仅作为建议值。
- 制度依据：部门考评总人数关联对应绩效等级强制分布比例得出各等级评级人数；即使人员绩效得分集中在同一区间，也需按对应绩效等级评级比例进行强制排名分布。

### 9.2 双向/多向汇报

- 双向/多向汇报包括一个员工有多个评价上级，也包括架构归属与实际业务线/项目归属不一致的情况。
- 工作占比按考核周期配置，由管理员维护；项目制场景可扩展为从项目关系自动带出并由管理员确认。
- 多个上级分别评分后，最终分数按工作占比权重加权汇总。
- 多个上级可以互相查看彼此评分。
- 绩效名额占用员工架构所属部门/中心名额，不按项目部门占用。

### 9.3 目标权重与附加项

- 考核周期内计划权重合计必须为 100%。
- “下季度目标计划”权重合计必须为 100%；本周期/上季度完成情况也按该周期计划权重 100% 口径校验。
- 加分、扣分等附加项单独维护，不参与目标权重合计。

### 9.4 报表

- 报表范围包括考核进度、员工目标完成情况、考核结果、部门等级分布、历史对比。
- 报表筛选维度包括公司、中心、部门、岗位、员工、周期、流程状态、等级。

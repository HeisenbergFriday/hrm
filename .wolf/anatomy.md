...
- `build-and-deploy.ps1` — !internal (~2762 tok)
...
- `PerformanceActivityEditor.tsx` — activitySections (~8338 tok)

## frontend/src/config/

- `menu.tsx` — menuPermissionKey (~1790 tok)
...
- `PerformanceIndicatorLibrary.tsx` — isValidWeight (~12568 tok)
...
- `PerformanceOverview.tsx` — PERFORMANCE_VIEW_OPTIONS (~37718 tok)
...
- `dingtalk_test.go` — TestBuildCorpMessagePayloadUsesAsyncSendSchema, TestShouldValidateAttendanceGroupMembersUsesExplicit (~734 tok)
- `dingtalk.go` — Struct: AppConfig (~30554 tok)
...
- `attendance_service.go` — Interface: attendanceRepository (~3054 tok)
- `department_service.go` — Struct: DepartmentService (~495 tok)
- `employee_service.go` — Struct: EmployeeService (~707 tok)
- `org_service.go` — Struct: OrgDataScope (~16534 tok)
...
- `cli.py` — cmd_leave, cmd_overtime, cmd_subsidy, cmd_final (~3264 tok)

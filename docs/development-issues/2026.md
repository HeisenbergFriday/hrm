...
| 2026-08-18 同根因复发 | 审批历史对账 Go 测试使用 `time.Local` 构造 18:00、21:00 打卡。本机 Windows/UTC+8 通过，但 GitHub Ubuntu/UTC 将这些绝对时间落到审批的 UTC+8 加班窗口外，两轮 Backend tests 均稳定失败。修复为使用固定 `attendanceCST` 构造测试数据，不修改业务逻辑；定向用例通过，GitHub Backend tests 待新提交复验。 |

...
### 2026-09-14 P1 People 容器日志无上限导致磁盘告警

| 字段 | 内容 |
|---|---|
| 日期 | 2026-09-14 |
| 级别 | P1 |
| 模块/标签 | `deploy` `logging` `docker` `gorm` `retention` |
| 范围 | People 主服务、DingTalk Stream、测试/生产 Docker Compose 与宿主机日志占用 |
| 现象 | 测试主容器连续运行 12 天后单个 JSON 日志约 17.8G，平均约 1.4G/天，触发服务器资源告警；容器检查显示 `json-file` 配置为空，没有大小或文件数上限。 |
| 根因 | Compose 未配置 Docker 日志轮转；应用对每个 HTTP 请求同时输出 Gin 访问日志和请求摘要；GORM 默认逐条输出正常 SQL 及完整 SQL 文本；应用没有按自然日归档和过期清理机制。 |
| 修复 | 测试与生产 Compose 为全部 People 容器配置 `json-file` 的 `100m × 10` 兜底上限。主服务与 Stream 统一写入挂载的 `logs/`，按 UTC+8 自然日分文件，启动、跨日写入及每天零点删除 7 天前文件。移除重复 Gin 访问日志；正常 SQL 只累计请求计数，错误及超过 1 秒的慢 SQL 仅记录 request_id、路由、表名、耗时、行数和错误摘要，不输出完整 SQL；Logrus 默认级别收口为 `warn`。 |
| 验证 | 日志组件单测覆盖第 7 个自然日保留、第 8 天删除；`go test ./internal/logging ./internal/database ./internal/api ./cmd ./cmd/dingtalk_stream -count=1`、对应 `go vet`、`golangci-lint`（0 issues）、测试/生产 `docker compose config`、`git diff --check` 均通过；服务器只读确认现有 4 个 People 测试容器均未配置轮转，未读取日志正文、未清理日志、未重启或部署。 |
| 防复发 | 1. 所有 People Compose 服务必须显式设置日志驱动的 `max-size` 和 `max-file`，禁止依赖宿主机默认值。<br>2. 需要固定天数的日志必须按业务时区自然日分文件，并由应用定时清理；容量轮转不能冒充日期保留。<br>3. 每个 HTTP 请求只保留一条摘要；正常 SQL 不落完整文本，错误/慢 SQL 只写脱敏元数据。<br>4. 应用日志目录必须使用持久化挂载，主服务与 Stream 使用不同文件名前缀。<br>5. 发布后必须确认容器 LogConfig、`logs/` 当日文件、午夜清理和磁盘增长率。 |
| 状态 | fixed（代码与本地验证完成，待部署后进行服务器验收） |

### 2026-09-11 P2 审批实例返回重置列表状态且缺少全量时间排序

| 字段 | 内容 |
|---|---|
| 日期 | 2026-09-11 |
| 级别 | P2 |
| 模块/标签 | `approval-instance` `frontend` `api-contract` `pagination` `navigation` `business-time` `detail-ui` |
| 范围 | 审批管理→审批实例列表、审批详情返回和 `/approvals/instances` 分页查询 |
| 现象 | 在审批实例列表筛选或搜索后进入详情，返回时筛选条件、分页和列表位置被重置；列表只有发起时间，无法直接看到各审批类型的业务开始/结束时间；审批完成时间名称不清晰，详情内容层级混杂且空字段较多；接口无法按业务时间对全部审批数据排序；重新部署后业务时间字段可能显示为空，日期范围仍按审批发起时间筛选。 |
| 根因 | 列表状态只存在组件内存，进入详情后没有可恢复的列表地址；排序字段未形成完整的前后端白名单契约；业务开始/结束时间保存在不同审批类型的表单 JSON 中，没有统一提取为列表响应字段；详情页把概览、表单和审批流放在同一层级，未过滤空值；Table 同时回调分页和变更事件时，分页事件携带的默认排序状态被误判为排序操作，导致页码被重置为 1。 |
| 修复 | 将筛选、关键词、分页、每页条数、排序字段/方向编码到审批实例列表 URL；进入详情前保存滚动位置，详情按钮和浏览器后退均返回原查询地址并在列表数据加载后恢复滚动；后端白名单支持 `create_time`、`finish_time`、`business_start_time`、`business_end_time`，业务时间从既有表单 JSON 提取并先对全量结果排序再分页，非法参数回退发起时间倒序；列表增加业务开始/结束时间列并将结束时间改名为审批完成时间；详情重排为概览、业务时间、审批内容和审批流程卡片，过滤空值；前端按 Table `extra.action` 区分分页和排序，分页切换不再触发排序逻辑；日期范围先按状态、流程、标题等条件取数，再按业务开始/结束时间提取后过滤并重新计算总数。 |
| 验证 | 审批实例前端定向测试 15/15 通过；审批 Repository 审批相关测试通过；TypeScript `tsc --noEmit`、定向 ESLint、前端生产构建和 `git diff --check` 均通过；回归覆盖点击第 2 页后请求 `page=2`，以及 create_time 与业务时间不同日期时的业务日期筛选。 |
| 防复发 | 1. 列表进入详情的返回契约必须携带可重建列表的查询地址，不能只依赖组件内存。<br>2. 需要跨页排序时，排序字段和方向必须进入后端查询契约，并在 `OFFSET/LIMIT` 前执行。<br>3. 用户可见的排序字段必须使用后端白名单，非法值不得进入 SQL。<br>4. 回归至少覆盖页面内返回、浏览器后退、筛选/分页/排序恢复、结束时间升降序和全量排序后分页。<br>5. Table 统一变更回调必须按 `extra.action` 区分 `paginate` 与 `sort`，分页事件不得执行排序状态切换或重置页码。<br>6. 业务日期筛选必须复用列表业务时间提取逻辑，不能重新按 `create_time` 过滤；缺少对应业务开始/结束时间的记录不应被错误纳入日期范围。 |
| 状态 | fixed（代码与定向验证完成，待真实页面验收） |

### 2026-09-02 P1 最终表考勤身份覆盖清空花名册人事字段

| 字段 | 内容 |
|---|---|
| 日期 | 2026-09-02 |
| 级别 | P1 |
| 模块/标签 | `attendance-toolbox` `final` `roster` `identity-merge` `data-contract` |
| 范围 | 考勤工具箱最终汇总：在职/离职花名册与钉钉月度考勤身份合并、最终表人事字段及入/离职口径 |
| 现象 | 花名册可解析出合同主体、员工类型、人员分类、入职日期、离职日期和转正日期，但最终表这些字段全部为空；未被月度考勤表匹配的花名册员工连工号、部门和岗位也会被清空，导致入/离职缺勤和当月转正天数无法按真实日期计算。 |
| 根因 | `apply_attendance_identity` 在匹配考勤身份前将花名册记录的身份与人事字段统一重置为空，再仅回填月度考勤表实际提供的工号、姓名、考勤组、部门和岗位；月度考勤表不提供合同主体及完整人事日期，因此权威花名册值被无条件丢弃。 |
| 修复 | 合并改为以花名册员工记录为基础；未匹配考勤身份时完整保留花名册数据，匹配时仅使用月度考勤表中的非空工号、姓名、考勤组、部门和岗位补充/纠正；合同主体、员工类型、人员分类、入职日期、离职日期和转正日期始终保留花名册值。同步更新最终表身份合并单测、runner 端到端测试及考勤模块长期规则。 |
| 验证 | 身份合并定向测试 12/12、最终表回归 75/75、runner 测试 18/18 通过，`py_compile`、`git diff --check` 通过。使用真实 8 月输入生成后，代码规则范围内输出 312 人：员工类型 312、人员分类 271、入职日期 310、离职日期 41、转正日期 271 条非空；公式错误 0。<br>2026-09-20 提交前复验：同步 `D:\app` 与 `tools/attendance-processing` 的最终表实现及 `excel_compat.py` 依赖；Python 工具箱 202/202、Go 相关包全量通过，最终表 golden fingerprint 与 live source compare 均恢复一致。 |
| 防复发 | 1. 花名册是合同主体、人员类型和人事日期的主数据源，身份补充必须从花名册副本开始，禁止先清空再回填。<br>2. 月度考勤表只允许以非空值更新其真实提供的工号、姓名、考勤组、部门和岗位；空值不得覆盖花名册。<br>3. 回归必须同时覆盖精确工号匹配、旧工号按唯一姓名回退、未匹配员工保留、重名歧义拒绝及最终工作簿人事字段写出。<br>4. 入/离职缺勤、人员筛选和当月转正天数依赖日期字段，身份合并测试必须与最终表端到端测试联动。 |
| 状态 | fixed（仓库代码、测试、真实文件验证已完成） |

### 2026-09-01 P1 兼职作息公式与固定人员规则导致出勤漏算

| 字段 | 内容 |
|---|---|
| 日期 | 2026-09-01 |
| 级别 | P1 |
| 模块/标签 | `attendance-toolbox` `parttime` `schedule-formula` `fixed-staff` `day-column` |
| 范围 | 考勤工具箱兼职汇总与最终汇总：默认作息表、简易考勤矩阵、固定人员日明细与提醒、最终表应出勤天数 |
| 现象 | 8 月兼职汇总的 15 日、29 日出勤和部分异常提醒缺失；固定人员只生成首周少数天，总天数与作息表不一致；“员工考勤表”无工号列时无法作为考勤明细输入。 |
| 根因 | 作息表第三周起用 Excel 公式递推日期，解析器按公式文本读取，导致后续工作日无法转换为日期；最终汇总的作息解析同样按公式文本读取，将深圳主作息误判为 6 天；固定人员分支直接返回作息日，未合并打卡异常；考勤明细校验强制要求工号且只支持标准双层表头，未识别简易日期矩阵及 `√` 标记。 |
| 修复 | 兼职默认作息与最终汇总作息均按 `data_only` 读取公式缓存值；最终汇总汇总行数值兼容 C/D 列，避免主作息 24 被漏读；固定名单收口为王心英、刘芮、汤颖、周代林、陈富庆，统一按主作息表计天，打卡异常合并为提醒且不改变 `1` 天；考勤明细在“姓名+至少 3 个日期列”时允许无工号矩阵，支持 `√` 和正数出勤标记。 |
| 验证 | 定向回归 15/15 通过；最终表作息解析回归与真实 `8月作息表.xlsx` 均验证为深圳 24 天、成都 21 天；用 6 份真实 8 月输入生成兼职汇总，固定 5 人均为 24 天且 15/29 日为 `1`，普通人员 15/29 日正常打卡为 `1` 且无新增提醒；工作表公式错误扫描为 0，已完成全表视觉检查。请假与兼职业务真源已备份并同步到 `D:\app`；全量 Python 套件另因当前运行时缺少 `chinese_calendar`、工作区既有 overtime/subsidy/final 真源差异与 final 输出指纹差异未全通过，不影响本次兼职定向结论。 |
| 防复发 | 1. 作息表日期可为公式，依赖日期值的计算必须读取 Excel 已计算结果并覆盖跨周/月末边界。<br>2. 固定人员的“出勤来源”与“异常来源”必须分开：作息表决定值，打卡只决定提醒，禁止用打卡异常改写作息值。<br>3. 放宽表头时必须使用“姓名+日期列数”等结构门槛，禁止对任意无工号文件 fail-open。<br>4. 回归必须覆盖月中与月末周六（如 15/29）、公式日期、固定人员异常不扣减及简易 `√` 矩阵。 |
| 状态 | fixed（兼职与最终汇总作息解析代码、定向测试、真实文件验证已完成；最终汇总部署侧源文件待同步） |

### 2026-08-20 P1 跨月请假结束月时长漏算或少算

| 字段 | 内容 |
|---|---|
| 日期 | 2026-08-20 |
| 级别 | P1 |
| 模块/标签 | `attendance-toolbox` `leave` `cross-month` `duration-allocation` `python` |
| 范围 | 请假明细月度计算、最终汇总离职计薪日明细、旧兼容入口与 `D:\app` 业务真源 |
| 现象 | 普通请假跨月时，起始月可按作息得到正确时长，但结束月再次独立按作息计算；边界时间含钉钉分钟偏差或结束日未在普通作息中时，结束月可能得到 3.5 小时或 0 小时，而系统总时长要求分摊为 4 小时，导致跨月合计少于原审批时长。 |
| 根因 | 跨月数字小时记录只裁剪目标月后按作息重算，没有把钉钉系统时长作为跨月总账，也没有“各月合计必须等于系统时长”的守恒校验；最终汇总的离职计薪日明细又独立按作息拆天，可能再次丢失请假明细已经分到结束月的时长。 |
| 修复 | 为普通数字小时制、跨相邻两个月的记录新增统一日分摊：按日期和边界时间生成基础小时，以系统时长校正总额，差额小于 8 小时时从结束日向前吸收，再按月份汇总；输出备注“跨月按系统时长分摊”。差异达到 8 小时或无法守恒时回退原作息结果并提示人工确认。产假、自然日假、文字“N天”、同月记录和人工覆盖保持原逻辑。最终汇总日明细复用同一日分摊。同步工具箱、旧兼容副本和 `D:\app` 请假/最终表业务真源，并更新请假核心哈希。 |
| 验证 | 新增 7 个跨月用例，覆盖 2026-07-30 至 2026-08-01 的 20 小时分摊（7 月 16 小时、8 月 4 小时）、结束日非普通工作日、同月不回归、自然日假、文字天数、跨年边界、差异过大人工确认及最终汇总日明细复用；请假与最终表相关回归 84/84 通过，Python 语法编译通过。工具箱完整套件执行 196 项，其中 194 项通过；2 项被本次范围外既有差异阻塞：兼职模块哈希漂移，以及 `D:\app` 最终表 52 列与工具箱 53 列的历史输出差异。`D:\app` 与 `tools/attendance-processing` 的本次两个业务文件 LF 哈希一致。 |
| 防复发 | 1. 跨月审批必须明确总账字段和分摊字段，月度结果之和必须等于源系统总时长。<br>2. 边界时间舍入差只允许在可解释范围内校正，差异达到 8 小时必须提示人工核对，禁止静默编造。<br>3. 月度汇总与离职计薪日明细必须复用同一分摊结果，禁止下游再次独立重算。<br>4. 特殊假期和文字天数不得误入数字小时分摊。<br>5. 覆盖月末、年末、周末边界以及同月对照用例；业务真源、兼容副本和核心哈希同步更新。 |
| 状态 | fixed（目标逻辑与相关回归通过；完整套件存在两项本次范围外既有阻塞） |
